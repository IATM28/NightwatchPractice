  ...
  /*  'Step 2: Select option "Sell a home"': function(browser) {
    ...
    'Step 3: Fill List information': function(browser) {
            ...
            browser
                ...
                //('select[name="state"]')
                //.assert.containsText('select[name="state"]', 'Arizona')
                //.click('select[name="state"] option[value="AZ"]')
                .pause(2000)
                .click('input[type="submit"]')
                .pause(2000)
                .assert.containsText('body', 'AZ')
                .getText('ul li:nth-child(1) p:nth-child(3)', function(result) {
                    dir = (JSON.stringify(result.value)).substr(8);
                    console.log('result: ' + (JSON.stringify(result.value)).substr(8));
                    browser.url('https://deqa.channelauction.com/homes/list-a-home/search')
                        .waitForElementVisible('body', 1000)
